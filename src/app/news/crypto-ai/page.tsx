...
export default function CryptoAINewsPage() {
  const [articles, setArticles] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Carica articoli Crypto AI dal database
    fetch('/api/news?category=CRYPTO_AI&limit=20')
      .then(res => res.json())
      .then(data => {
        setArticles(data);
        setLoading(false);
      ...
      .catch(err => {
        console.error('Errore caricamento articoli Crypto AI:', err);
        setLoading(false);
      });
  }, []);
    ...
    <div className="relative z-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 flex items-center justify-center shrink-0">
            <Image
              src={imparoLogo}
              alt="Crypto AI"
              width={28}
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Crypto AI News</h1>
            <p className="text-slate-600 dark:text-slate-400">Intelligenza artificiale e blockchain unite</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-indigo-500/20 bg-white dark:bg-indigo-900/25 p-10 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Nessun articolo Crypto AI</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-6">
              Non ci sono ancora articoli pubblicati nella categoria Crypto AI.
            </p>
            <Link
              href="/news"
              className="inline-flex bg-cyan-600 text-white px-6 py-3 rounded-lg hover:bg-cyan-700 transition-colors font-medium"
            >
              Torna alle News
            </Link>
          </div>
