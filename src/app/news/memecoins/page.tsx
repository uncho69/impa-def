...
export default function MemecoinsNewsPage() {
  const [articles, setArticles] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Carica articoli Memecoins dal database
    fetch('/api/news?category=MEMECOINS&limit=20')
      .then(res => res.json())
      .then(data => {
        setArticles(data);
        setLoading(false);
      ...
      .catch(err => {
        console.error('Errore caricamento articoli Memecoins:', err);
        setLoading(false);
      });
  }, []);
