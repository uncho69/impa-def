...
export const tweets = pgTable('tweets', {
    ...
    isVerified: smallint('is_verified'), // null = not checked, 1 = verified, -1 = rejected
    verifiedAt: timestamp('verified_at'),
    verifiedBy: varchar('verified_by', { length: 50 }), // 'system', 'manual', etc.
    isActive: smallint('is_active').notNull().default(0),
    deletedAt: timestamp('deleted_at'),
    createdBy: varchar('created_by', { length: 50 }),
    updatedBy: varchar('updated_by', { length: 50 }),
    ...
    check('tweets_impressions_check', sql`${table.impressions} >= 0`),
    check('tweets_is_verified_check', sql`${table.isVerified} IS NULL OR ${table.isVerified} IN (-1, 1)`),
